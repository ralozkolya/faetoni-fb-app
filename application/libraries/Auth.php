...
class Auth {
	...
	public function change_current_user($username, $password) {

		if($this->is_logged_in()) {
			$username_old = $this->CI->session->userdata(USER)[USERNAME];

			$this->change_password($username_old, $password);

			$this->CI->db->where(array(USERNAME => $username_old));

			return $this->CI->db->update(USERS, array(USERNAME => $username));
		}
	}
